...
public class ExtentReportManager implements ITestListener {
	...
	public void onStart(ITestContext testContext) {
		...
		repName = "ZigWheel Report " + timeStamp + ".html";
		sparkReporter = new ExtentSparkReporter(".\\reports\\" + repName);// specify location of the report

		sparkReporter.config().setDocumentTitle(" ZigWheels Automation Report"); // Title of report
		sparkReporter.config().setReportName("ZigWheels Testing"); // name of the report
		sparkReporter.config().setTheme(Theme.DARK);

		extent = new ExtentReports();
		extent.attachReporter(sparkReporter);
		extent.setSystemInfo("Application", "ZigWheels");
		extent.setSystemInfo("Module", "Identify Bikes");
		extent.setSystemInfo("Sub Module", "Login");
		extent.setSystemInfo("User Name", "Kedar Angolkar");//System.getProperty("user.name")
